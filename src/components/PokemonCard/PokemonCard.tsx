...
const PokemonCard: React.FC<Props> = ({ pokemon, onClick }) => {
    const handleMouseEnter = (e: React.MouseEvent<HTMLDivElement>) => {
        ...
        target.style.boxShadow = '0 15px 40px rgba(0, 0, 0, 0.4)';
        const img = target.querySelector('img');
        if (img) (img as HTMLImageElement).style.transform = 'scale(1.15) rotate(5deg)';
    };
    ...
    const handleMouseLeave = (e: React.MouseEvent<HTMLDivElement>) => {
        ...
        const img = target.querySelector('img');
        if (img) (img as HTMLImageElement).style.transform = 'scale(1) rotate(0deg)';
    };
    ...
    return (
        <div
            style={{
                ...
                // cast typeColors to indexable record to satisfy TS
                background: (typeColors as Record<string, string>)[(pokemon.type || 'normal').toLowerCase()] || typeColors.normal,
            }}
        ...
        >
            ...
            <img src={(pokemon.image as string) || ''} alt={pokemon.name} style={cardStyles.image} />
