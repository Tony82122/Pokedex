...
const PokemonModal: React.FC<Props> = ({ pokemon, isVisible, onClose }) => {
    if (!isVisible || !pokemon) return null;

    const handleCloseButtonHover = (e: React.MouseEvent<HTMLSpanElement>, isHovering: boolean) => {
        const target = e.currentTarget as HTMLSpanElement;
        target.style.backgroundColor = isHovering ? 'rgba(0,0,0,0.1)' : 'rgba(0,0,0,0.05)';
        target.style.transform = isHovering ? 'rotate(90deg)' : 'rotate(0deg)';
    };
    ...
    return (
        <div style={{ ...modalStyles.modal, display: 'flex' }} onClick={onClose}>
            ...
            >
                ...
                <div style={{ textAlign: 'center', marginBottom: '20px' }}>
                    <span
                        style={{
                            ...
                            fontSize: '16px',
                            fontWeight: '700',
                            background: typeColors[(pokemon.type || 'normal').toLowerCase()] || typeColors.normal,
                            color: 'white',
                            textTransform: 'capitalize',
                ...
                <img src={(pokemon.image as string) || ''} alt={pokemon.name} style={modalStyles.modalImage} />
